...
public class WayaPaymentDAOImpl implements WayaPaymentDAO {
    ...
    public void expireAllTransactionMoreThan30Mins() {
        ...
        String UPDATE_QUERY = "UPDATE m_payment_gateway SET transaction_expired=true WHERE transaction_expired=false AND EXTRACT(EPOCH FROM (CURRENT_TIMESTAMP - tran_date ))/60 >= 30.0000 ";
        ...
        String UPDATE_SANDBOX_QUERY = " UPDATE m_sandbox_payment_gateway SET transaction_expired=true WHERE transaction_expired = false AND EXTRACT(EPOCH FROM (CURRENT_TIMESTAMP - tran_date ))/60 >= 30.0000 ";
